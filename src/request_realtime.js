  ...
  async function pullAndExcel(){
    try{
     ...
     //从数据库中获取昨天天每次的Zcash收入，并求和，插入Zcashincome表
      ...
      let selectResult = await utils.P(utils.pool,'query',selectSameStartMinEnd);
      ...
      logger.logger.info('activeworker',activeworker)
       ...
       let dateresult =await utils.P(utils.pool,'query',datequery);
       if(dateresult.length!=0){
         ...
         await utils.P(utils.pool,'query',updatequery);
       }
       else{
      ...
      for(let i=0;i<data.length;i++){
        array[i] = [
              ...
              data[i].payee,
              data[i].description,
              (data[i].income).toFixed(3),
              data[i].outpay,
              (data[i].amount).toFixed(3),
              data[i].activeworker,
              (data[i].hashrate).toFixed(3),
              (data[i].in_hashrate_ratio).toFixed(3)
        ];
        ...
        let filePath = uploadDir + filename + exceldate + '.xlsx';
         ...
         //获取云币网的相关数据
         let yunbidata = await new Promise(function(resolve,reject){
